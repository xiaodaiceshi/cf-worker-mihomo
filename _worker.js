...
`);n=e,i=e+1,o=s[i]}return n}var pt=(s,e)=>({indentAtStart:e?s.indent.length:s.indentAtStart,lineWidth:s.options.lineWidth,minContentWidth:s.options.minContentWidth}),mt=s=>/^(%|---|\.\.\.)/m.test(s);function Hn(s,e,t){if(!e||e<0)return!1;let n=e-t,i=s.length;if(i<=n)return!1;for(let o=0,r=0;o<i;++o)if(s[o]===`
...
+(?!
|$)`,"g")}catch{Qt=/\n+(?!\n|$)/g}function dt({comment:s,type:e,value:t},n,i,o){let{blockQuote:r,commentString:a,lineWidth:l}=n.options;if(!r||/\n[\t ]+$/.test(t))return Le(t,n);let c=n.indent||(n.forceBlockIndent||mt(t)?"  ":""),p=r==="literal"?!0:r==="folded"||e===y.BLOCK_FOLDED?!1:e===y.BLOCK_LITERAL?!0:!Hn(t,l,c.length);if(!t)return p?`|
`:`>
...
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${c}`),A=!1,O=pt(n,!0);r!=="folded"&&e!==y.BLOCK_FOLDED&&(O.onOverflow=()=>{A=!0});let b=Ve(`${S}${v}${h}`,c,ft,O);if(!A)return`>${x}
...
`+A.comment:c.comment=A.comment),u=A.end;continue}!r&&t.options.strict&&ge(k)&&i(k,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line")}if(m===0)A.comma&&i(A.comma,"UNEXPECTED_TOKEN",`Unexpected , in ${a}`);else if(A.comma||i(A.start,"MISSING_CHAR",`Missing , between ${a} items`),A.comment){let O="";e:for(let b of S)switch(b.type){case"comma":case"space":break;case"comment":O=b.source.substring(1);break e;default:break e}if(O){let b=c.items[c.items.length-1];N(b)&&(b=b.value??b.key),b.comment?b.comment+=`
`+O:b.comment=O,A.comment=A.comment.substring(O.length+1)}}if(!r&&!x&&!A.found){let O=v?s(t,v,A,i):e(t,A.end,x,null,A,i);c.items.push(O),u=O.range[2],us(v)&&i(O.range,"BLOCK_IN_FLOW",cs)}else{t.atKey=!0;let O=A.end,b=k?s(t,k,A,i):e(t,O,S,null,A,i);us(k)&&i(b.range,"BLOCK_IN_FLOW",cs),t.atKey=!1;let T=ne(x??[],{flow:a,indicator:"map-value-ind",next:v,offset:b.range[2],onError:i,parentIndent:n.indent,startOnNewline:!1});if(T.found){if(!r&&!A.found&&t.options.strict){if(x)for(let M of x){if(M===T.found)break;if(M.type==="newline"){i(M,"MULTILINE_IMPLICIT_KEY","Implicit keys of flow sequence pairs need to be on a single line");break}}A.start<T.found.offset-1024&&i(T.found,"KEY_OVER_1024_CHARS","The : indicator must be at most 1024 chars after the start of an implicit flow sequence key")}}else v&&("source"in v&&v.source&&v.source[0]===":"?i(v,"MISSING_CHAR",`Missing space after : in ${a}`):i(T.start,"MISSING_CHAR",`Missing , or : between ${a} items`));let fe=v?s(t,v,T,i):T.found?e(t,T.end,x,null,T,i):null;fe?us(v)&&i(fe.range,"BLOCK_IN_FLOW",cs):T.comment&&(b.comment?b.comment+=`
`+T.comment:b.comment=T.comment);let xe=new C(b,fe);if(t.options.keepSourceTokens&&(xe.srcToken=w),r){let M=c;$t(t,M.items,b)&&i(O,"DUPLICATE_KEY","Map keys must be unique"),M.items.push(xe)}else{let M=new I(t.schema);M.flow=!0,M.items.push(xe);let As=(fe??b).range;M.range=[b.range[0],As[1],As[2]],c.items.push(M)}u=fe?fe.range[2]:T.end}}let f=r?"}":"]",[h,...g]=n.end,d=u;if(h&&h.source===f)d=h.offset+h.source.length;else{let m=a[0].toUpperCase()+a.substring(1),w=p?`${m} must end with a ${f}`:`${m} in block collection must be sufficiently indented and end with a ${f}`;i(u,p?"MISSING_CHAR":"BAD_INDENT",w),h&&h.source.length!==1&&g.unshift(h)}if(g.length>0){let m=ie(g,d,t.options.strict,i);m.comment&&(c.comment?c.comment+=`
...
${n}`:n;else if(i||e.directives.docStart||!o)e.commentBefore=n;else if(E(o)&&!o.flow&&o.items.length>0){let r=o.items[0];N(r)&&(r=r.key);let a=r.commentBefore;r.commentBefore=a?`${n}
